...
export interface DialLayout {
  ...
  name: string;
  /**
   * Glyph painted on the side strip beside this dial, so the physical controls
   * are not unlabelled. Drawn alone in a 60x90 band at s=40 — larger than the
   * 37.8px key-tile icons — so it must stay distinct from the tile glyphs
   * visible 60px away on the same panel. The centre wheel has no side strip.
   */
  strip?: IconName;
